...
inline constexpr int kDefaultBoardSize = 5;  // ????
inline constexpr int kMaxNeighbors = 6;  // Maximum number of neighbors for a cell
...
typedef std::vector<std::vector<uint16_t>> NeighborList;
...
class GeodesicYState : public State {
  ...
  struct Cell {
    ...
    Cell() {}
    Cell(GeodesicYPlayer player_, uint16_t parent_, uint8_t edge_)
        : player(player_), parent(parent_), size(1), edge(edge_) {}
  };

 public:
  GeodesicYState(std::shared_ptr<const Game> game, uint16_t board_size, bool ansi_color_output);
 ...
 private:
  ...
  NeighborList graph;
  std::vector<uint16_t> graph_left;
  std::vector<uint16_t> graph_bottom;
  std::vector<uint16_t> graph_right;
  const uint16_t board_size_;
